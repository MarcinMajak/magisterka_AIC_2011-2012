...
class GeneticRoughSetsClassifier(object):
    ...
    def __create_next_generation(self, generation):
        """
        """
        if generation in [800, 600, 450, 300, 250, 170, 100, 50]:
            self.__diversify()
...
if __name__ == '__main__':
    ...
    DEBUG = False
    result_file = 'results/genetic_rough_sets_classifier.csv'
    try:
    ...
    for d in range(len(datasets)):
        for k in range(K_FOLD_NUMBER):
            for r in range(REPEAT_TEST):
                ...
                if grs.read_data(filepath=filename, label_is_last=(bool)(datasets[d][1])) == False:
                ...
                grs.create_population(population_size=POPULATION_SIZE, division=MAX_GRANULATION)
                grs.run()
                
                size = len(grs.testing_label)
                classified = 0
                for i in range(size):
                    if grs.testing_label[i] == grs.classify(grs.testing_data[i]):
                        classified += 1
                        
                # calculate how many attributes are used in the best classification
                active_attributes = 0
                for a in range(grs.get_number_of_attributes()):
                    if not grs.the_classification[a] == grs.DO_NOT_USE:
                        active_attributes += 1
                        
                print "Finished iteration %d for %s dataset" % (r, datasets[d][0]) 
                fd.write("%s,%d,%d,%d,%d,%d,%d\n" % (datasets[d][0], r, k, size, classified, grs.get_number_of_attributes(), active_attributes))
                fd.flush()
    fd.close()