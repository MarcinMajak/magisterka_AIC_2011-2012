...
class GeneticFuzzyLogicClassifier(object):
...
#            for i in range(attr_length):
#                if not rule[i] == self.NOT_USED:
#                    rule_number += 1
#        rule_number = rule_number / len(self.the_classification)
#        print "Srednia liczba atrybutow dla regul to %.2f " % rule_number
    ...
    def __create_next_generation(self, generation):
        """
        """
        if generation in [800, 600, 500, 350, 250, 150, 50]:
            self.__diversify()
    ...
    def __print_summary(self):
        ...
        print "Dla tego rozpoznania funkcje wygladaly tak"
        attr_length = self.get_number_of_attributes()
        print "[",
        for rule in self.the_classification:
            print "[",
...
if __name__ == '__main__':
    ...
    DEBUG = False
    result_file = 'results/genetic_fuzzy_logic_classifier.csv'
    try:
    ...
    for d in range(len(datasets)):
        for k in range(K_FOLD_NUMBER):
            for r in range(REPEAT_TEST):
                filename = 'datasets/%s' % datasets[d][0]
                fuzzy = GeneticFuzzyLogicClassifier(debug=DEBUG)
                if fuzzy.read_data(filepath=filename, label_is_last=(bool)(datasets[d][1])) == False:
                    print "Error with opening the file. Probably you have given wrong path"
                    sys.exit(1)
                fuzzy.prepare_data(k_fold_number=K_FOLD_NUMBER)
                fuzzy.k_fold_cross_validation(k=k)
                fuzzy.generate_membership_functions(divisions=DIVISIONS)
                fuzzy.initialize_genetic(generations=500, mutation=0.3, crossover=0.9)
                fuzzy.create_population(population_size=POPULATION_SIZE)
                fuzzy.run()
                
                size = len(fuzzy.testing_label)
                classified = 0
                for i in range(size):
                    if fuzzy.testing_label[i] == fuzzy.classify(fuzzy.testing_data[i]):
                        classified += 1
                            
                active_attributes = 0
                for rule in fuzzy.the_classification:
                    for i in range(fuzzy.get_number_of_attributes()):
                        if not rule[i] == fuzzy.NOT_USED:
                            active_attributes += 1
                active_attributes = (float)(active_attributes / fuzzy.get_number_of_attributes())
                
                print "Finished iteration %d for %s dataset" % (r, datasets[d][0])
                fd.write("%s,%d,%d,%d,%d,%d,%.2f\n" % (datasets[d][0], r, k, size, classified, fuzzy.get_number_of_attributes(), active_attributes))
                fd.flush()
